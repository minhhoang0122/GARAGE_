...
import { Wrench, ArrowRight, UserCheck, AlertCircle, Loader2 } from 'lucide-react';
...
export default function AssignOverviewPage() {
    ...
    const { data: jobs = [], isLoading: jobsLoading } = useQuery({
        queryKey: queryKeys.mechanic.assigned(),
        queryFn: () => mechanicService.getAssignedJobs(),
    });
    ...
    // Realtime update on SSE
    useRealtimeUpdate(queryKeys.mechanic.assigned());
    useRealtimeUpdate(queryKeys.mechanic.available());

    const isLoading = jobsLoading || mechLoading;

    const currentUserId = session?.user?.id ? parseInt(session.user.id) : null;
    const activeJobs = jobs;

    // Tách: đơn chưa nhận vs đơn đã nhận
    const unclaimedJobs = activeJobs.filter((j: any) => !j.claimedById);
    const claimedJobs = activeJobs.filter((j: any) => j.claimedById);
            ...
            <div className="relative z-10 max-w-6xl mx-auto pb-10 space-y-8">
                ...
                {isLoading ? (
                    <div className="flex flex-col items-center justify-center py-20 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
                        ...
                        <p className="text-slate-500 font-medium">Đang tải danh sách điều phối...</p>
                    </div>
                ) : (
                    <>
                        {/* Đơn chờ nhận */}
                                        ...
                                        return (
                                            <div key={job.id} className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-5 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden flex flex-col justify-between group">
                                                <div className={`absolute top-0 left-0 w-1 h-full ${
                                                    isInProgress(job.status) ? 'bg-blue-500' : 'bg-emerald-500'
                                                }`}></div>
                                                <div>
                                                    <div className="flex justify-between items-start mb-1">
                                                        ...
                                                        <span className={`px-2.5 py-1 text-xs font-medium rounded-full whitespace-nowrap ${
                                                            ...
                                                            {isInProgress(job.status) ? 'Đang sửa' : 'Đã nhận'}
                                                        </span>
                                                    </div>
                                                ...
                                                <div className="mt-4 pt-3 border-t border-slate-100 dark:border-slate-700 flex justify-between items-center">
                                                    ...
                                                    >
                                                        {isInProgress(job.status) ? 'Điều chỉnh nhân sự' : 'Vào phân công'} <ArrowRight className="h-4 w-4" />
                                                    </Link>
                                                </div>
                                            </div>
                                        );
