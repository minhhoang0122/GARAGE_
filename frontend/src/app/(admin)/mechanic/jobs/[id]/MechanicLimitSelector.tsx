...
export default function MechanicLimitSelector({ itemId, currentCount, maxLimit = 4, isLead }: MechanicLimitSelectorProps) {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (res && res.success) {
                ...
                toast({
                    title: "Lỗi",
                    description: res?.error || 'Lỗi cập nhật',
                    variant: 'destructive'
                });
            }
        } catch (error: any) {
    ...
    if (!isLead) {
        return (
            <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border ${currentCount >= maxLimit ? "bg-red-50 text-red-600 border-red-200" : "bg-slate-100 text-slate-600 border-slate-200 dark:bg-slate-800 dark:text-slate-400 dark:border-slate-700"}`}>
                <Users className="w-3.5 h-3.5" />
                <span>{currentCount}/{maxLimit}</span>
            </div>
        );
    }
        ...
        return (
            <div className="flex items-center gap-1 bg-white dark:bg-slate-800 border rounded-md p-1 animate-in fade-in zoom-in duration-200 shadow-sm">
                ...
                <input
                    ...
                    autoFocus
                />
                <Button size="icon" variant="ghost" className="h-6 w-6 text-emerald-600 hover:text-emerald-700 hover:bg-emerald-50 cursor-pointer" onClick={handleSave} disabled={isLoading}>
                    {isLoading ? <Loader2 className="w-3 h-3 animate-spin" /> : <Check className="w-3 h-3" />}
                </Button>
                <Button size="icon" variant="ghost" className="h-6 w-6 text-slate-400 hover:text-slate-500 hover:bg-slate-50" onClick={() => setIsEditing(false)}>
                    <X className="w-3 h-3" />
                </Button>
            </div>
        );
