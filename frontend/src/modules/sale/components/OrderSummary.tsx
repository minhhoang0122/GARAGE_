...
import { useUpdateOrderTotals } from '@/modules/sale/hooks/useSale';
import { useRouter } from 'next/navigation';
import { useToast } from '@/modules/shared/components/ui/use-toast';
import { usePermission } from '@/hooks/usePermission';
import { useOrderWorkspaceOptional } from './OrderWorkspaceProvider';

import { isRejected, isCancelled } from '@/lib/status';
...
export default function OrderSummary({
    ...
    const { mutate: updateTotalsMatch, isPending: isSaving } = useUpdateOrderTotals();
    ...
    const [editData, setEditData] = useState<{ discount: number; vatPercent: number | string }>({
        ...
        const apiCall = async () => {
            updateTotalsMatch({
                ...
                onSuccess: () => {
                    toast({
                        title: "Thành công",
                        description: "Đã cập nhật tổng hợp báo giá",
                    });
                    setIsEditing(false);
                },
                onError: (error: any) => {
                            ...
                            <div className="relative w-24">
                                <input
                                    ...
                                    onChange={(e) => {
                                        ...
                                        if (val === '' || /^\d+$/.test(val)) {
                                            setEditData(prev => ({ ...prev, vatPercent: val }));
                                        }
                                    }}
