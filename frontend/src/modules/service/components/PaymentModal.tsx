...
function getDefaultType(orderStatus: string, amountPaid: number, remainAmount: number) {
    ...
    const postRepairStatuses = ['CHO_THANH_TOAN', 'HOAN_THANH', 'DONG'];
    if (postRepairStatuses.includes(orderStatus)) return 'PAYMENT';

    const preRepairStatuses = ['TIEP_NHAN', 'BAO_GIA', 'CHO_KH_DUYET', 'DA_DUYET'];
    if (preRepairStatuses.includes(orderStatus)) return 'DEPOSIT';
